      ...
      <div>
        ...
        <div className="space-y-2">
          {topCampuses.slice(0, 10).map((c) => (
            <div
              key={c.rank}
              className="flex items-center justify-between rounded p-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-700/50"
            >
              <div className="flex min-w-0 items-center">
                <span className="w-5 shrink-0 font-bold text-slate-400 dark:text-slate-500">{c.rank}</span>
                <span className="w-24 truncate font-bold text-slate-700 dark:text-slate-300">{c.name}</span>
                {c.type && (
                  <span
              ...
              <div className="flex shrink-0 items-center gap-3">
                <span className="font-bold tabular-nums text-slate-800 dark:text-white">{c.pScore}</span>
